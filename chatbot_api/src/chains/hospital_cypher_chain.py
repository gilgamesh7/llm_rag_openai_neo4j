# Neo4j Cypher chain will 
#   accept a user’s natural language query, 
#   convert the natural language query to a Cypher query, 
#   run the Cypher query in Neo4j, 
#   and use the Cypher query results to respond to the user’s query. 
...
# LangChain wrapper that allows LLMs to execute queries on your Neo4j instance
graph = Neo4jGraph(
...
# This is where you’re providing the LLM with very specific instructions on 
# what it should and shouldn’t do when generating Cypher queries. 
# Most importantly, you’re showing the LLM your graph’s structure with t
# he schema parameter, 
# some example queries, 
# and the categorical values of a few node properties.
...
# define the prompt template for the question-answer component of your chain. 
# This template tells the LLM to use the Cypher query results 
# to generate a nicely-formatted answer to the user’s query
...
# instantiate a GraphCypherQAChain object
hospital_cypher_chain = GraphCypherQAChain.from_llm(
    cypher_llm=ChatOpenAI(model_name=HOSPITAL_CYPHER_MODEL, temperature=0),
    qa_llm=ChatOpenAI(model_name=HOSPITAL_QA_MODEL, temperature=0),
    graph=graph,
    qa_prompt=qa_prompt,
